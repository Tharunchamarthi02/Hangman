...
public:
    ...
    void encryptWord(const char* word, char* guess) {
        int i = 1;
        guess[0] = word[0];
        while (word[i] != '\0') {
            guess[i] = '*';
    ...
    bool checkGuess(const char* word, char* guess, char ch) {
        bool check = false;
        int i = 0;
        ch = toupper(ch);
        char temp;
    ...
    bool checkWord(const char* word, char* guess) {
        int i = 0;
        while (word[i] != '\0') {
            if (word[i] != guess[i])
                return false;
    ...
    void storetheword(const char* x) {
        response = x;
    }
};
